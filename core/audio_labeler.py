...
def label_audio(work_dir: str, script_path: str, engine: str = 'fish', max_retries: int = 2) -> Optional[List[Dict]]:
    with open(script_path, 'r', encoding='utf-8') as f:
        raw_text = f.read()

    if engine == 'omnivoice':
        prompt = f"""你是一个专业的语音合成预处理助手。请将以下口播稿文本转换为适合 OmniVoice 模型的格式。

要求：
1. 根据语义将全文分割为自然段落，每段不超过 350 字。
2. **多音字改为拼音注音**：对多音字，将其改为大写拼音和声调（如“打折”改为“打ZHE2”、“折本”改为“SHE2本”、“折腾”改为“ZHE1腾”）。注意：只有多音字需要注音。
3. 输出格式：纯文本，段落之间用一个空行分隔。
...
2. 模拟人类朗读的换气位置，在需换气位置加入[inhale]。换气应模拟人类朗读的自然呼吸停顿，不要在每个自然段都加，而是根据语义、情感和句子长度合理放置。
3. 根据情感变化，在适当位置插入情感标签（如 `[激昂]`、`[冷静]`、`[严肃]`、`[自豪]` 等）。情感标签应放在该句或该段之前，且每个标签不超过 4 个字。
4. 对于突发情感变化（如大喊），应在该句前单独添加标签（如 `[大喊]`）。
5. 输出格式：纯文本，标签用方括号，文本正常书写。段落之间用且仅用一个空行分隔，段落内部不要出现空行。标签与文字之间用空格分隔即可。
