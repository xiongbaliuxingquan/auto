...
2. 从全文提取全局资产库：角色的固定属性（如外貌、气质、核心设定）、整体视觉风格。全局资产库用文本列表形式，每个资产项以“- 键：值”格式输出，放在“【全局资产库】”下方。
   ...
   **重要：角色固定属性必须严格按照以下格式输出，每个字段都要包含，且描述必须明确、肯定，不得使用“或”等模棱两可的词语。性别字段只能为“男”或“女”。如果原文没有明确，请根据上下文推断最可能的一种，并直接写出。**
   格式：
   【角色名】 种族：X，性别：X，年龄：X，发型：X，发色：X，脸型：X，身高：X，体型：X，惯用着装：X，气质描述：X。
   注意：种族可以是“人类”、“熊猫”、“机器人”、“精灵”等。

   示例：
   - 角色固定属性：
     【阿铁】 种族：人类，性别：男，年龄：少年，发型：短发，发色：黑，脸型：圆脸，身高：中等，体型：瘦削，惯用着装：深色短打工匠服，气质描述：坚韧、热血、对锻造充满纯粹热爱。
     【阿六】 种族：熊猫，性别：男，年龄：青年，发型：短圆耳，发色：黑白相间，脸型：圆脸，身高：中等，体型：圆润敦实，惯用着装：磨损的深色背带工装裤，肩部有简易护甲，左前肢为机械义肢，佩戴护目镜，气质描述：沉稳、坚韧、沉默中带着不容动摇的执着。

3. 为每个段落提取局部资产库：该段落的场景、角色服装变化、道具、当前情绪、时间、地域等。局部资产也用文本列表形式，每个资产项以“- 键：值”格式输出。

   如果指定了目标总时长，请在【局部资产】中添加一行“- 建议时长：X秒”，表示该段落应该占用的时长。

**特别要求：所有涉及时代背景的描述（包括整体视觉风格、角色着装、场景、地域）都必须明确写出具体的朝代/历史时期（如“明代”、“唐代”等）。**
   - 整体视觉风格中必须包含“历史年代：X”字段，例如“历史年代：明代，标准电影镜头...”。
   - 角色固定属性中的“惯用着装”必须写明朝代，例如“明代月白色绫袄”。
   - 局部资产中的“场景”描述必须包含朝代，例如“明代应天府灯市”。
   - “地域”字段必须按“国家·时代”格式填写，例如“中国·明代”。
...
- 整体视觉风格：...
- 角色固定属性：
  【角色名】 种族：...，性别：...，年龄：...，发型：...，发色：...，脸型：...，身高：...，体型：...，惯用着装：...，气质描述：...
...
"""
    ...
    def _generate_script_for_paragraph(self, idx: int, text: str, local_assets_text: str, prev_summary: str, suggested_duration: int, target_duration_minutes: int, log) -> Dict:
        ...
        # 调整镜头时长以符合段落配额
        if scene and suggested_duration > 0:
            total_duration = sum(shot.get('duration', 0) for shot in scene['shots'])
            if abs(total_duration - suggested_duration) > 2:
                scale = suggested_duration / total_duration
                for shot in scene['shots']:
                    shot['duration'] = round(shot.get('duration', 0) * scale, 1)
                # 使用已有的 log 函数（该方法内可用 log）
                if log:
                    log(f"段落 {paragraph_index} 镜头总时长从 {total_duration:.1f}s 调整为 {suggested_duration:.1f}s")
        if not scene:
...
"""
        ...
        if suggested_duration > 0:
            duration_instruction = f"""
**严格规定：此段落的目标时长配额为 {suggested_duration} 秒。**
